...
- `#` in the command is replaced with the output file path
...
The command must write a JSON object to the output file (`#`):
...
3. Runs the command with `@` replaced by the unpacked path and `#` replaced by the output file path
4. Reads the result JSON from the output file and compares the predicted status against the ground truth from the companion `.yaml`
