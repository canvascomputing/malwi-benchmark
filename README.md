...
|----------|-------------|
| `<command> @ #` | The shell command to evaluate. `@` is replaced with the path to each unpacked sample. `#` is replaced with the output file path. |
...
# Real tool
make run "../agentwerk/target/debug/malware-scanner @ --output #" claude-sonnet-4-20250514 malware-scanner
```
...
3. Run the command with `@` replaced by the unpacked sample path and `#` replaced by the output file path.
4. Read the result JSON from the output file and compare the predicted status against the ground-truth label from the companion `.yaml`.
...
The command under evaluation must write a JSON object to the output file (`#`):
...
|-------|------|-------------|
| `status` | string | Predicted status: `malicious`, `exploitable`, or `benign` |
